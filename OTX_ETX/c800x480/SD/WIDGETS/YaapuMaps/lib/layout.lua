...
local function drawMiniHud(x,y)
  libs.drawLib.drawArtificialHorizon(x, y, 92, 70, nil, lcd.RGB(0x7B, 0x9D, 0xFF), lcd.RGB(0x63, 0x30, 0x00), 6, 6.5, 1.3)
  lcd.drawBitmap(utils.getBitmap("hud_92x70"), 6, 70)
end
...
function layout.draw(widget,battery,alarms,frame,customSensors,gpsStatuses,leftPanel,centerPanel,rightPanel)
  ...
  if conf.sidebarEnable == true then
    ...
    lcd.drawFilledRectangle(LCD_W-100, 28, 100, LCD_H, CUSTOM_COLOR)
  end

  if conf.bottombarEnable == true then
    lcd.setColor(CUSTOM_COLOR, utils.colors.darkgrey)
    lcd.drawFilledRectangle(0, LCD_H-50, LCD_W, 50, CUSTOM_COLOR)
  end
