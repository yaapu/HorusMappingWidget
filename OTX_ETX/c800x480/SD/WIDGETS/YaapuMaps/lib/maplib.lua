...
local function google_coord_to_tiles(lat, lng, level)
  ...
  return math.floor(rx/140), math.floor(ry/140), math.floor(rx%140), math.floor(ry%140)
end
...
local function gmapcatcher_coord_to_tiles(lat, lon, level)
  ...
  return math.floor(x % world_tiles), math.floor(y % world_tiles), math.floor((x - math.floor(x)) * 140), math.floor((y - math.floor(y)) * 140)
end
...
-- Per una griglia 4x4, width/2 = 2. 
    ...
    local margin = 140 * 0.1 -- 10% di margine per evitare oscillazioni
    
    -- Se l'offset è molto vicino ai bordi del tile centrale, allora slitta
    if offset_y < margin then
        yy = yy + 1
    elseif offset_y > (140 - margin) then
        yy = yy - 1
    end

    if offset_x < margin then
        xx = xx + 1
    elseif offset_x > (140 - margin) then
        xx = xx - 1
    end
  for x=1,TILES_X
  do
...
local function drawTilesFixed(width, color, level, offset_x, offset_y)
  ...
  local start_x = (LCD_W / 2) - ((xx - 1) * 140) - offset_x
  local start_y = (LCD_H / 2) - ((yy - 1) * 140) - offset_y
  ...
  for x=1, TILES_X do
    for y=1, TILES_Y do
      ...
      if tiles[idx] ~= nil then
        ...
        local posX = start_x + (x - 1) * 140
        local posY = start_y + (y - 1) * 140
        
        -- Ottimizzazione: disegna solo se il tile è almeno parzialmente visibile
        if posX > -140 and posX < LCD_W and posY > -140 and posY < LCD_H then
          lcd.drawBitmap(getTileBitmap(tiles[idx]), posX, posY, 140)
        end
      end
    end
  end
...
local function drawTiles(width,xmin,xmax,ymin,ymax,color,level)
  ...
  do
    ...
    do
      local idx = width*(y-1)+x
      if tiles[idx] ~= nil then
        lcd.drawBitmap(getTileBitmap(tiles[idx]), xmin+(x-1)*140, ymin+(y-1)*140, 140)
      end
    end
  end
  if conf.enableMapGrid then
    ...
    for x=1,TILES_X-1
    do
      lcd.drawLine(xmin+x*140,ymin,xmin+x*140,ymax,DOTTED,color)
    end

    for y=1,TILES_Y-1
    do
      lcd.drawLine(xmin,ymin+y*140,xmax,ymin+y*140,DOTTED,color)
    end
  end
  lcd.drawBitmap(utils.getBitmap("maps_box"), 0, ymax-21)
  -- draw 50m or 150ft line at max zoom
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawLine(xmin+7,ymax-2,xmin+5+scaleLen,ymax-2,SOLID,CUSTOM_COLOR)
  lcd.drawText(xmin+7,ymax-22,string.format("%s (%d)",scaleLabel,level),SMLSIZE+CUSTOM_COLOR)
  ...
  if telemetry.lon ~= nil and telemetry.lat ~= nil then
    ...
    if telemetry.homeLat ~= nil and telemetry.homeLon ~= nil then
      ...
      lcd.drawBitmap(utils.getBitmap("minihomeorange"),xmax-24,ymax-20)
    else
      utils.drawBlinkBitmap("minihomeorange",xmax-24,ymax-20)
    end
    lcd.drawText(xmax-32, ymax-22, utils.decToDMSFull(telemetry.lat).." "..utils.decToDMSFull(telemetry.lon,telemetry.lat),CUSTOM_COLOR+RIGHT)
  end
end
...
local function getScreenCoordinates(minX,minY,tile_x,tile_y,offset_x,offset_y,level)
  ...
  if tcache ~= nil then
    if tiles[tcache[1]] ~= nil then
      -- ok it's on screen
      return minX + (tcache[2]-1)*140 + offset_x, minY + (tcache[3]-1)*140 + offset_y
    end
  end
  -- force offscreen up
  return LCD_W/2, -10
...
local function setupMaps(x, y, level, tiles_x, tiles_y)
  ...
  if level ~= lastZoomLevel then
    ...
    if conf.mapProvider == 1 then
      ...
      scaleLen = ((unitScale==1 and 1 or 3)*50*2^(level+2)/tile_dim)*140
    elseif conf.mapProvider == 2 then
      ...
      scaleLen = ((unitScale==1 and 1 or 3)*50*2^(20-level)/tile_dim)*140
...
function mapLib.drawMap(widget, x, y, w, h, level, tiles_x, tiles_y)
  ...
  local maxY = math.min(LCD_H, math.min(y+h, minY+TILES_Y*140))

  local minX = math.max(0, x)
  local maxX = math.min(LCD_W, math.min(x+w, minX+TILES_X*140))
  ...
  if telemetry.lat ~= nil and telemetry.lon ~= nil then
    ...
    if myScreenX ~= nil then
      lcd.setColor(CUSTOM_COLOR,utils.colors.white)
      libs.drawLib.drawRArrow(myScreenX,myScreenY,30-5,telemetry.yaw,CUSTOM_COLOR)
      lcd.setColor(CUSTOM_COLOR,utils.colors.black)
      libs.drawLib.drawRArrow(myScreenX,myScreenY,30,telemetry.yaw,CUSTOM_COLOR)
    end
    ...
    do
      if p ~= (sampleCount-1)%conf.mapTrailDots then
        ...
        if tilesXYByPath[posHistory[p][1]] ~= nil then
          ...
          lcd.drawFilledRectangle(minX + (x-1)*140 + posHistory[p][2]-1, minY + (y-1)*140 + posHistory[p][3]-1,3,3,CUSTOM_COLOR)
        end
      end
    end
  end
  if telemetry.lat ~= nil and telemetry.lon ~= nil then
    ...
    lcd.drawText(minX+50,minY+10+3,"hdg",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(minX+170,minY+10+3,"gspd",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(minX+320,minY+10+3,"home",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(maxX-140,minY+10+3,"travel",SMLSIZE+CUSTOM_COLOR+RIGHT)
    -- values
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(minX+50+3,minY+10,string.format("%d%s",telemetry.yaw,utils.degSymbol),CUSTOM_COLOR)
    lcd.drawText(minX+170+3,minY+10,status.avgSpeed.value*conf.horSpeedMultiplier >=10 and string.format("%.0f%s",status.avgSpeed.value*conf.horSpeedMultiplier, conf.horSpeedLabel) or string.format("%.01f%s",status.avgSpeed.value*conf.horSpeedMultiplier, conf.horSpeedLabel) ,CUSTOM_COLOR)
    lcd.drawText(minX+320+3,minY+10,string.format("%.0f%s",telemetry.homeDist*unitScale, unitLabel),CUSTOM_COLOR)
    lcd.drawText(maxX-140+3,minY+10,string.format("%.01f%s",status.avgSpeed.travelDist*unitLongScale, unitLongLabel),CUSTOM_COLOR)
    
    -- home
    lcd.setColor(CUSTOM_COLOR,utils.colors.yellow)
    libs.drawLib.drawRArrow(maxX-60,maxY-65,1.5*30,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
    lcd.setColor(CUSTOM_COLOR,utils.colors.black)
    libs.drawLib.drawRArrow(maxX-60,maxY-65,1.7*30,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
  end
