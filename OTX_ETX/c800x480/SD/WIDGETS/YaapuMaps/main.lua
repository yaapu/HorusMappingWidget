...
local function drawRssi()
  ...
  lcd.drawText(620, 0, strRSSI, RIGHT+CUSTOM_COLOR)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(620 - 180 , 0, string.format("RTP: %d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), RIGHT+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(620, 0, string.format("RS: %d/%d", telemetry.rssiCRSF, getValue("RFMD")), RIGHT+CUSTOM_COLOR+SMLSIZE)
end
...
function utils.drawTopBar(widget)
  ...
  if status.modelString ~= nil then
    lcd.drawText(2, 0, status.modelString, CUSTOM_COLOR+0)
  end
  ...
  if telemetryEnabled(widget) == false then
    ...
    lcd.drawText(620-23, 0, "RS:---", RIGHT+CUSTOM_COLOR)
  ...
  lcd.drawText(640,0, vtx, SMLSIZE+CUSTOM_COLOR)
end
