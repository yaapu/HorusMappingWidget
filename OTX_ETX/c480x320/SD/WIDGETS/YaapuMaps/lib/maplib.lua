...
function mapLib.drawMap(widget, x, y, w, h, level, tiles_x, tiles_y)
  ...
  if telemetry.lat ~= nil and telemetry.lon ~= nil then
    ...
    lcd.drawText(minX+25,minY+4+3,"hdg",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(minX+100,minY+4+3,"gspd",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(minX+200,minY+4+3,"home",SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.drawText(maxX-95,minY+4+3,"travel",SMLSIZE+CUSTOM_COLOR+RIGHT)
    -- values
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(minX+25+3,minY+4,string.format("%d%s",telemetry.yaw,utils.degSymbol),CUSTOM_COLOR)
    lcd.drawText(minX+100+3,minY+4,status.avgSpeed.value*conf.horSpeedMultiplier >=10 and string.format("%.0f%s",status.avgSpeed.value*conf.horSpeedMultiplier, conf.horSpeedLabel) or string.format("%.01f%s",status.avgSpeed.value*conf.horSpeedMultiplier, conf.horSpeedLabel) ,CUSTOM_COLOR)
    lcd.drawText(minX+200+3,minY+4,string.format("%.0f%s",telemetry.homeDist*unitScale, unitLabel),CUSTOM_COLOR)
    lcd.drawText(maxX-95+3,minY+4,string.format("%.01f%s",status.avgSpeed.travelDist*unitLongScale, unitLongLabel),CUSTOM_COLOR)
