...
local function drawRssi()
  ...
  lcd.drawText(345, 0, strRSSI, RIGHT+CUSTOM_COLOR)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(345 - 80 , 0, string.format("RTP: %d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), RIGHT+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(345, 0, string.format("RS: %d/%d", telemetry.rssiCRSF, getValue("RFMD")), RIGHT+CUSTOM_COLOR+SMLSIZE)
end
...
function utils.drawTopBar(widget)
  ...
  if telemetryEnabled(widget) == false then
    ...
    lcd.drawText(345-23, 0, "RS:---", RIGHT+CUSTOM_COLOR)
  ...
  lcd.drawText(360,0, vtx, 0+CUSTOM_COLOR)
end
