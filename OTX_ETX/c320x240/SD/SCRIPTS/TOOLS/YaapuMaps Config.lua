...
local function drawConfigMenuBars()
  ...
  lcd.drawRectangle(0, 0, LCD_W, 14, CUSTOM_COLOR)
  lcd.drawFilledRectangle(0,LCD_H-12, LCD_W, 14, CUSTOM_COLOR)
  lcd.drawRectangle(0, LCD_H-12, LCD_W, 14, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(0,0, model.getInfo().name, CUSTOM_COLOR)
  lcd.drawText(LCD_W,0,"Yaapu Mapping Widget 2.2.0 dev".."( "..'f5eb634'..")",SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.drawText(2,LCD_H-12+-2,getConfigFilename(),CUSTOM_COLOR)
  lcd.drawText(LCD_W,LCD_H-12+-2,itemIdx,CUSTOM_COLOR+RIGHT)
end
...
local function drawItem(idx,flags)
  ...
  if menuItems[idx][2] == 0 then
    if menuItems[idx][4] == 0 and menuItems[idx][5] >= 0 then
      lcd.drawText(180,14 + (idx-menu.offset-1)*15, "---",flags+CUSTOM_COLOR)
    else
      lcd.drawNumber(180,14 + (idx-menu.offset-1)*15, menuItems[idx][4],flags+menuItems[idx][8]+CUSTOM_COLOR)
      if menuItems[idx][7] ~= nil then
        lcd.drawText(180 + 50,14 + (idx-menu.offset-1)*15, menuItems[idx][7],flags+CUSTOM_COLOR)
      end
    end
  else
    lcd.drawText(180,14 + (idx-menu.offset-1)*15, menuItems[idx][5][menuItems[idx][4]],flags+CUSTOM_COLOR)
  end
end
...
local function drawConfigMenu(event)
  ...
  elseif menu.selectedItem  < 1 then
    ...
    --menu.offset =  #menuItems - 14
    menu.offset = #menuItems > 14 and #menuItems - 14 or 0
  end
  --
  for m=1+menu.offset,math.min(#menuItems,14+menu.offset) do
    lcd.setColor(CUSTOM_COLOR,WHITE)
    lcd.drawText(2,14 + (m-menu.offset-1)*15, menuItems[m][1],CUSTOM_COLOR)
