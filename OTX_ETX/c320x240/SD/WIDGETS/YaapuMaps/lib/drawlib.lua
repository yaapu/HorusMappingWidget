...
function drawLib.drawNoTelemetryData(telemetryEnabled)
  if (not telemetryEnabled()) then
    ...
    lcd.drawFilledRectangle(30,80, 260, 80, CUSTOM_COLOR)
    lcd.setColor(CUSTOM_COLOR,utils.colors.red)
    lcd.drawFilledRectangle(30+2,80+2, 260-4, 80-4, CUSTOM_COLOR)
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(math.floor(LCD_W/2), 80+10, "no telemetry data", DBLSIZE+CUSTOM_COLOR+CENTER)
    lcd.drawText(math.floor(LCD_W/2), 80+50, "Yaapu Mapping Widget 2.2.0 dev".."( "..'300cb39'..")", SMLSIZE+CUSTOM_COLOR+CENTER)
  end
end
...
function drawLib.drawNoGPSData()
  if telemetry.lat == nil or telemetry.lon == nil then
    ...
    lcd.drawFilledRectangle(30+2,80+2, 260-4, 80-4, CUSTOM_COLOR)
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(math.floor(LCD_W/2), 80+10, "...waiting for GPS", DBLSIZE+CUSTOM_COLOR+CENTER)
    lcd.drawText(math.floor(LCD_W/2), 80+50, "Yaapu Mapping Widget 2.2.0 dev".."( "..'300cb39'..")", SMLSIZE+CUSTOM_COLOR+CENTER)
    return true
  end
  return false
...
function drawLib.drawArtificialHorizon(x, y, w, h, bgBitmapName, colorSky, colorTerrain, lineCount, lineOffset, scale)
  ...
  if telemetry.roll == 0 then
    drawLib.drawFilledRectangle(minX,math.max(minY,dy+minY+(maxY-minY)/2),maxX-minX,math.max(0,math.min(maxY-minY,(maxY-minY)/2-dy+(math.abs(dy) > 0 and 1 or 0))),CUSTOM_COLOR)
