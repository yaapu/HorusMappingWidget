...
local function drawRssi()
  ...
  lcd.drawText(255, 0, strRSSI, RIGHT+CUSTOM_COLOR)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(255 - 60 , 0, string.format("RTP:%d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), RIGHT+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(255, 0, string.format("RS:%d/%d", telemetry.rssiCRSF, getValue("RFMD")), RIGHT+CUSTOM_COLOR+SMLSIZE)
end
...
function utils.drawTopBar(widget)
  ...
  local strtime = string.format("%02d:%02d",time.hour,time.min,time.sec)
  ...
  if telemetryEnabled(widget) == false then
    ...
    lcd.drawText(255-23, 0, "RS:---", RIGHT+CUSTOM_COLOR)
  ...
  lcd.drawText(260,0, vtx, SMLSIZE+CUSTOM_COLOR)
end
