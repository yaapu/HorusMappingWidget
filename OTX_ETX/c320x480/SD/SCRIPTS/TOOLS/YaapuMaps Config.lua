...
local menuItems = {
  ...
  {"enable top panel:", 1, "TBE", 2, { "yes", "no" }, { true, false } },
...
local function applyConfigValues(conf)
  ...
  conf.sidebarEnable = false
  conf.topbarEnable = getMenuItemByName(menuItems,"TBE")
...
local function drawConfigMenuBars()
  ...
  lcd.drawText(LCD_W,2,"Yaapu Mapping Widget 2.2.0 dev".."( "..'2ffdaf8'..")",SMLSIZE+CUSTOM_COLOR+RIGHT)
...
local function drawItem(idx,flags)
  ...
  if menuItems[idx][2] == 0 then
    if menuItems[idx][4] == 0 and menuItems[idx][5] >= 0 then
      ...
      lcd.drawNumber(215,25 + (idx-menu.offset-1)*20, menuItems[idx][4],flags+menuItems[idx][8]+CUSTOM_COLOR)
      if menuItems[idx][7] ~= nil then
        lcd.drawText(215 + 50,25 + (idx-menu.offset-1)*20, menuItems[idx][7],flags+CUSTOM_COLOR)
      end
    end
  else
    lcd.drawText(215,25 + (idx-menu.offset-1)*20, menuItems[idx][5][menuItems[idx][4]],flags+CUSTOM_COLOR)
  end
end
...
local function drawConfigMenu(event)
  ...
  elseif menu.selectedItem  < 1 then
    ...
    menu.offset = #menuItems > 19 and #menuItems - 19 or 0
  end
  --
